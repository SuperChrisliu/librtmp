...
public class RtmpHeader {
    ...
    public static RtmpHeader readHeader(ByteBuf in, RtmpClient rtmpClient) throws IOException {
        RtmpHeader rtmpHeader = new RtmpHeader();
        rtmpHeader.readHeaderImpl(in, rtmpClient);
        return rtmpHeader;
    }

    private void readHeaderImpl(ByteBuf in, RtmpClient rtmpClient) throws IOException {
        int basicHeaderByte = in.readByte();
        ...
        switch (chunkType) {
            case TYPE_0_FULL: { //  b00 = 12 byte header (full header) 
                ...
                messageType = MessageType.valueOf(in.readByte());
                // Read bytes 8-11: Message stream ID (apparently little-endian order)
                byte[] messageStreamIdBytes = new byte[4];
                in.readBytes(messageStreamIdBytes, 0, messageStreamIdBytes.length);
            ...
            case TYPE_1_RELATIVE_LARGE: { // b01 = 8 bytes - like type 0. not including message stream ID (4 last bytes)
                ...
                packetLength = Util.readUnsignedInt24(in);
                // Read byte 7: Message type ID
                messageType = MessageType.valueOf((byte) in.readByte());
                // Read bytes 1-4: Extended timestamp delta
                extendedTimestamp = timestampDelta >= 0xffffff ? Util.readUnsignedInt32(in) : 0;
                RtmpHeader prevHeader = rtmpClient.getChunkStreamInfo(chunkStreamId).prevHeaderRx();
                if (prevHeader != null) {
